...
Layout: 280px left sidebar (search input + "New chat" button pinned at top, conversation list grouped under "Today" / "Yesterday" / "Previous 7 days" headers below) + a centered main chat column, max-width 760px, with a scrollable message list (24px vertical gap between message groups) and a composer pinned to the bottom of the viewport (auto-growing textarea supporting 1-6 visible lines, send button, model/tool selector as a small dropdown to its left).

Visual hierarchy: The current message thread is the sole focus — sidebar text is smaller (text-sm) and muted (text-white/50) except the active conversation, which gets a filled background and full-opacity text. The composer is visually elevated (a subtle top border plus a background one step lighter than the page) but never exceeds ~20% of viewport height, so at least 3-4 messages stay visible above it at all times.

Components & states: Sidebar conversation items (title truncated to one line, relative timestamp, active/selected state with a filled pill background); user vs. assistant message bubbles with distinct alignment and styling — assistant messages are left-aligned, no bubble, full-width prose with markdown support (headings, lists, and fenced code blocks in a monospace font with a language label and a copy button); user messages are right-aligned, max-width 70%, in a {{primaryColor}}-tinted rounded bubble. The composer's send button is disabled while the input is empty and swaps to a stop/cancel icon while a response is streaming; an inline streaming indicator (animated cursor or three-dot pulse) appears at the end of the in-progress assistant message rather than as a separate loading row. A "scroll to latest" pill button appears only when the user has scrolled up during an active stream.

Design language: Dark, low-glare background suited for long reading sessions (avoid pure black; use a card-tier surface one step lighter for the composer). Use {{primaryColor}} only for the user message bubble, the active sidebar item, the send button, and the copy-button success state — assistant responses and code blocks stay neutral/high-contrast for readability. Generous line-height (1.6+) and a comfortable line length (65-75 characters) on message text; code blocks use a slightly smaller monospace size than surrounding prose.

Responsive behavior: Sidebar collapses to an overlay drawer below 768px, toggled from a slim top bar showing the current conversation title and a menu icon; the composer stays fixed to the bottom of the viewport at every breakpoint, with its max-width shrinking alongside the chat column rather than going full-bleed on tablet widths.

Accessibility: New assistant message content is announced via aria-live="polite" in incremental, sentence-sized chunks as it streams — not on every character; the composer's send/stop button has a clear disabled state and an accessible name that changes with its mode ("Send message" / "Stop generating"); conversation list items and code-block copy buttons are keyboard-navigable via Tab with a visible focus-visible outline; code blocks expose their language to screen readers via a visually-hidden label.`,
  },
  {
