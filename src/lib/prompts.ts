...
export const prompts: PromptTheme[] = [
  {
    ...
    promptTemplate: `Product context: A B2B SaaS analytics dashboard used by operations teams to monitor product usage and revenue metrics throughout the workday. Optimize for fast daily scanning, not first-time delight.
...
Visual hierarchy: Page title (text-2xl, font-semibold) sits above a single row of 4 KPI cards — these are the primary focal point and should use the largest numerals on the page (text-3xl, tabular-nums). Charts are secondary (text-sm section labels, text-lg titles); the activity table is tertiary and scrolls independently rather than competing for vertical space. Never let body copy or table text exceed the visual weight of the KPI numbers.

Components & states: 4 KPI cards (label, value, trend badge with up/down icon and percentage, sparkline); a line chart (usage over time, with hover tooltip and a visible empty state for zero-data ranges); a bar chart (revenue by segment, with a loading skeleton matching its final aspect ratio); a paginated, sortable activity table with a loading skeleton, empty state, and zebra-striped or bordered rows for scanability. Buttons and nav items need explicit default/hover/active/disabled states.
...
Accessibility: Target WCAG AA contrast (4.5:1 body text, 3:1 large text/UI) for {{primaryColor}} against the dark background — pick a lighter tint if the base value fails at small sizes. Icon-only nav items and buttons get descriptive aria-labels; the activity table uses real <table> markup with <th scope="col"> headers and is fully operable via keyboard (Tab/Shift+Tab between rows, Enter to open a row); all interactive elements show a visible focus-visible outline distinct from hover state.`,
  },
  {
    ...
    promptTemplate: `Product context: The above-the-fold hero section of a startup's marketing landing page. Its only job is to make a first-time visitor understand the product's value proposition and take one action within 3 seconds of landing.
...
Components & states: Eyebrow badge (pill shape, subtle border, small dot or icon); headline with one {{primaryColor}}-highlighted phrase; subtext paragraph (1-2 sentences, no more); a primary CTA button (filled, {{primaryColor}} background) with explicit hover (slight brightness/scale change) and focus states, plus a secondary ghost/text link beside it; a logo strip of 5-6 grayscale partner/customer logos; a decorative background layer (gradient blob or dot-grid) that never overlaps or reduces contrast behind the text.
...
Accessibility: Logo strip images need descriptive alt text (client name); hover-only image zoom must never hide text needed to understand a case study; all CTAs have specific, descriptive accessible names.`,
  },
  {
    ...
    promptTemplate: `Product context: A conversational AI assistant product used daily for extended work sessions — prioritize readability of long message threads and fast access to past conversations.

Layout: 280px left sidebar (search + "New chat" button + conversation history grouped by date) + a centered main chat column (max-width ~760px) with a scrollable message list and a fixed-bottom composer (auto-growing textarea, send button, model/tool selector).

Visual hierarchy: The current message thread is the sole focus — sidebar text is smaller and muted; the composer is visually elevated but never taller than ~20% of viewport height so message history stays visible.

Components & states: Sidebar conversation items (title, timestamp, active/selected state); user vs. assistant message bubbles with distinct alignment/background (assistant left-aligned, no bubble; user right-aligned, {{primaryColor}}-tinted bubble); composer with send button (disabled while empty or while a response streams) and a typing/streaming indicator.

Design language: Dark, low-glare background suited for long reading sessions. Use {{primaryColor}} only for the user message bubble, the active sidebar item, and the send button — assistant responses stay neutral/high-contrast for readability. Generous line-height (1.6+) and a comfortable line length on message text.

Responsive behavior: Sidebar collapses to an overlay drawer below 768px, toggled from a slim top bar; the composer stays fixed to the bottom of the viewport at every breakpoint.

Accessibility: New assistant messages are announced via aria-live="polite" as a response streams in; the composer's send button has a clear disabled state and accessible name; conversation list items are keyboard-navigable via Tab.`,
  },
  {
