---
title: "Examples"
source: "https://ui.shadcn.com/docs/registry/examples?utm_source=chatgpt.com"
author:
  - "[[shadcn]]"
published:
created: 2026-07-06
...
Use `files[].target` placeholders when a registry item should install files under the user's configured shadcn directories. The available placeholders are `@components/`, `@ui/`, `@lib/` and `@hooks/`.

The placeholders are resolved from `components.json`, so the same registry item works in projects using `@/`, custom TypeScript aliases, package imports or workspace package exports.

Anything after the placeholder is preserved. For example, `@ui/ai/prompt-input.tsx` installs under the user's configured `ui` directory at `ai/prompt-input.tsx`.
...
The `target` controls where the file is written, even when it differs from the file `type`.
